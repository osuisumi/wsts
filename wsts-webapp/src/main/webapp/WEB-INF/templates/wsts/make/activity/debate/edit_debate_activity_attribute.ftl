<#macro editDebateActivityAttributeFtl activity>
	<form id="saveActivityAttributeForm" action="${ctx }/make/activity/${(activity.id)!}" method="put">
		<ul class="g-addElement-lst g-addCourse-lst">
			<li class="m-addElement-item">
				<div class="ltxt">发表观点(次数)：</div>
				<div class="center">
					<div class="m-pbMod-ipt">
						<input type="text" name="activity.attributeMap[debate_view_num].attrValue" placeholder="输入次数" class="u-pbIpt {digits:true, min:0}" value="${(activity.attributeMap.debate_view_num.attrValue)!}">
					</div>
				</div>
			</li>
