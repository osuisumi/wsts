<#macro editEvaluateFtl relationId type>
	<@lcecEvaluateDirective lcecId=relationId getEvaluateSubmissions='Y'>
		<#if evaluateSubmissions??>
			<#if (evaluateSubmissions?size>0)>
	...
	<@evaluateRelation relationId=relationId type=type>
	    ...
	    <form id="createEvaluateItemForm" action="${ctx }/mark/evaluate/item"  method="post">
	    	<input type="hidden" name="evaluate.id" value="${evaluate.id }">
	    	<input type="hidden" name="content">
	    </form> 
	    <form id="updateEvaluateItemForm" action="${ctx }/mark/evaluate/item/"  method="put">
	    	<input type="hidden" name="content">
	    </form> 
	</@evaluateRelation>
	<script>
		  ...
		  //章节活动拖拽排序
			$("#evaluateItemUl").sortable({
		        ...
		        stop: function(){
		        	...
		        	$.put('${ctx}/mark/evaluate/item', data);
		        }
		    }).disableSelection();
		});
		//添加评分项
		    ...
		    if(bindParent === '' || bindParent === null || bindParent === 'undefined' || bindParent === undefined){
		        ...
		        $('.txtitemBox').on('click','.m-ltxt-lst .u-delete',function(){
		        	...
		        	confirm('确定要删除此评分项吗?' , function(){
			            $.ajaxDelete('${ctx }/make/evaluate/item/'+itemId, '', function(data){
