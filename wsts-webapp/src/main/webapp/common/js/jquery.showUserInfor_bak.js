    ...
    Plugin.prototype.init = function(){
        ...
        var userInformationHtml = '<div id="showUserInformation" style="position: absolute; z-index: 99999; width: 224px; height: 128px; background-color: #fff; border: 1px solid #e1e1e1;" data-in="false">'+
                                        ...
                                        '<div class="opt" style="height: 40px; line-height: 40px; border-top: 1px solid #e1e1e1; text-align: center;">'+
                                            '<div class="followDiv" style="float: left; width: 48%; text-align: center;"></div>'+
        ...
        $element.on('mouseenter',function(){
            ...
            //加载关注状态
            var followDiv = $userInformation.find('.followDiv');
            //设置用户id和登录人id
            followDiv.attr('userId',userId);
            followDiv.attr('loginId',loginId);
            followDiv.attr('ctx',ctx);
            if(userId != loginId){
...
function goUserCenter(a){
	...
	var ctx = $(a).attr('ctx');
	window.location.href = ctx+"/userCenter/zone/index?userId="+userId;
}
